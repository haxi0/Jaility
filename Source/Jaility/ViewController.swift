...
class ViewController: UIViewController {
    ...
    @IBAction func modernOSDownloadB(_ sender: Any) {
        ...
        self.modernOSDownloadBIB.setTitle("Wait", for: UIControl.State.normal)
        self.modernOSDownloadBIB.isUserInteractionEnabled = false
        FileDownloader.loadFileAsync(url: url!) { (path, error) in
            print(".zip File downloaded to : \(path!)")
            self.modernOSDownloadBIB.setTitle("Installing", for: UIControl.State.normal)
            ...
            do {
                        if FileManager.default.fileExists(atPath: myurl.path) {
                            ...
                            self.modernOSDownloadBIB.isUserInteractionEnabled = true
                        }
                    } catch {
                        print("oops")
                        self.modernOSDownloadBIB.setTitle("Installed (?)", for: UIControl.State.normal)
    ...
    @IBAction func RemoveCachedThemesModernOS(_ sender: Any) {
        let fileManager = FileManager.default
        do {
            try fileManager.removeItem(atPath: "/var/mobile/Library/WebClips/modernOS.zip")
            let alert = UIAlertController(title: "Success!", message: "The cached themes were removed successfully. You can download the themes to your device now again.", preferredStyle: .alert)
            alert.addAction(UIAlertAction(title: "OK", style: .default, handler: nil))
            self.present(alert, animated: true)
        } catch {
            print("error")
            let alert = UIAlertController(title: "Error!", message: "The cached themes were not removed successfully. There was an error deleting the cache.", preferredStyle: .alert)
            alert.addAction(UIAlertAction(title: "OK", style: .default, handler: nil))
            self.present(alert, animated: true)
        }
    }
    
    @IBAction func RemoveCachedFelicityPro(_ sender: Any) {
        let fileManager = FileManager.default
        do {
    ...
    @IBAction func FelicityProDownloadB(_ sender: Any) {
        ...
        self.FelicityProDownloadBIB.setTitle("Wait", for: UIControl.State.normal)
        self.FelicityProDownloadBIB.isUserInteractionEnabled = false
        FileDownloader.loadFileAsync(url: url!) { (path, error) in
            print(".zip File downloaded to : \(path!)")
            self.FelicityProDownloadBIB.setTitle("Installing", for: UIControl.State.normal)
            print(webClipsPath!.path)
            print(myurl.path)
            let fileManager = FileManager()
            var sourceURL = URL(fileURLWithPath: webClipsDecomP!.path)
            ...
            do {
                        if FileManager.default.fileExists(atPath: myurl.path) {
                            ...
                            self.FelicityProDownloadBIB.isUserInteractionEnabled = true
                        }
                    } catch {
                        print("oops")
                        self.modernOSDownloadBIB.setTitle("Installed (?)", for: UIControl.State.normal)
